...
xterm -hold -e  "sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /etc/apache2/ssl/apache.key -out /etc/apache2/ssl/apache.crt"
sh -c "sudo chmod -R 777 /etc/apache2/ssl/*"
bak
sh -c "sudo mv default-ssl.conf /etc/apache2/sites-enabled/default-ssl.conf"
sh -c "sudo /etc/init.d/apache2 reload"
